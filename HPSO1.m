...
for i=1:n   %function output at position
    fv(i,1)=(p(i,1)^2-20*cos(pi*p(i,1))+10)+(p(i,2)^2-20*cos(pi*p(i,2))+10);
    %Updating pbest
    if k==1
        pbestval(i,1) = fv(i,1);
...
for k=1:Kmax
    ...
    g(1,1) = (2*x(1) + 20*pi*sin(pi*x(1)));
    g(2,1) = (2*x(2) + 20*pi*sin(pi*x(2)));
    %Defining Hessian Matrix
    H(1,1) = (20*pi^2*cos(pi*x(1)) + 2 );
    H(1,2) = 0;
    H(2,1) = 0;
    H(2,2) = (20*pi^2*cos(pi*x(2)) + 2);
...
fv_N=(xf).^2-20*cos(pi*(xf)+10+(xf).^2-20*cos(pi*(xf))+10); %newton function value
